...
var items = [];
var nonItemPages = ['home','filter','list','search'];
var filters = ['plants','animals','hikes'];
// run once for the app
$(function() {
	/*document.addEventListener("backbutton", function(e){
	    if(!isItemPage ()){
	    	console.log('BACK, non-item-page');
	        e.preventDefault();
	        navigator.app.exitApp();
	    }
	    else {
	    	console.log('BACK, item-page');
	        navigator.app.backHistory();
	    }
	}, false);*/
	// initialization
	loadItems(); // load up the item thumbnails and pages
	setTimeout(function () {
		$.mobile.changePage("#filter", "fade");
	}, 4000);
	...
	$('#home .content h1, #home .content h2').click(function () {
		$.mobile.changePage("#filter", "fade");
	});
	$('#filter .content ul li a').click(function () {
		$(this).toggleClass('on');
	});
	// apply filtering when the go btn is pressed
	$('#filter .content #go-btn').click(function () {
		for (var i in filters) {
			if ($('#filter .content li.'+filters[i]+' a').hasClass('on')) {
				$('#list .content a.'+filters[i]).show();
			} else {
				$('#list .content a.'+filters[i]).hide();
			}
		}
	});
...
function loadItems() {
	$.each(itemData,function (i, item) {
		...
		$('#list .content').append('<a href="#'+item.id+'" title="'+item.title+'" class="'+item.tags.join(' ')+'"><img src="'+item.sm_img[0]+'" />'+listTitle.substring(0,14)+'</a>');
		...
		itemPgHtml += '<div data-role="page" id="'+item.id+'" class="item" data-title="Item Detail" data-theme="b" data-position="fixed">';
			itemPgHtml += '<div data-role="header" data-position="inline" data-theme="b">';
				itemPgHtml += '<a href="#list" data-direction="back" data-icon="back">Back</a>';
